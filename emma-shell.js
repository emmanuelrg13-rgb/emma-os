/* Emma OS v1.7.4 · 2026-09-03 · OpenAI/ChatGPT (GPT-5.5 Thinking)
   Cambio: Finanzas avanza a M4 paridad antes del corte. */
(function(){
  const script = document.currentScript;
  const base = script?.dataset?.base || './';
  const current = script?.dataset?.current || 'inicio';
  const version = script?.dataset?.version || 'v1.7.4';
  const apps = [
    ...
    {id:'finanzas', icon:'$', name:'Finanzas', desc:'Control financiero M4 paridad', href: base + 'apps/finanzas/', accent:'gold'},
